...
function amountToBaseUnits(value: number, decimals: number): string {
  if (!Number.isFinite(value) || value <= 0) throw new Error("trade amount must be greater than zero");
...
export async function executeJupiterXStockTrade(
  ...
  const inputDecimals =
    side === "buy"
      ? 6
      : (await getMint(connection, wallet.publicKey ? { toBase58: () => "" } as never : undefined as never));

  const decimals = side === "buy"
    ? 6
    : (await getMint(connection, new (await import("@solana/web3.js")).PublicKey(stockMint))).decimals;

  const amount = amountToBaseUnits(amountUsd, decimals);
  ...
  if (!quoteResponse.ok) {
    throw new Error(`jupiter_quote_failed:${quoteResponse.status}:${await quoteResponse.text()}`);
  }
  ...
  if (!swapResponse.ok) {
    throw new Error(`jupiter_swap_build_failed:${swapResponse.status}:${await swapResponse.text()}`);
  }
